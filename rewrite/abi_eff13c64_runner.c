...
void eff_13C64(void);
#pragma aux eff_13C64 __parm __caller [] __modify __exact [__ax]
void eff_13C64(void) {
    IplayRegs r;

    memset(&r, 0, sizeof(r));
    r.eax = input_ax;
    iplay_eff_13c64(&r, channel, input_byte_24668);
...
int main(int argc, char **argv) {
    ...
    if (argc != 5) return 2;
    if (!streq(argv[1], "abieff13c64")) return 2;

    memset(channel, 0, sizeof(channel));
    input_byte_24668 = (db)strtoul(argv[2], 0, 0);
    channel[0x3d] = (db)strtoul(argv[3], 0, 0);
    ...
    printf("ax=%04x data=%02x\n", ax_after, (unsigned)channel[0x3d]);
    return 0;
}
